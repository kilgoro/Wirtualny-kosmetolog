...
Virtual Beautician 💆‍♀️WordPress Plugin – An interactive cosmetology quiz with personalized treatment recommendations. The perfect tool for beauty salons, aesthetic clinics, and skin care studios.Features5-Step Quiz – Guides the client through body part selection, specific skin/body concerns, and preferred methods.4-Tab Admin Panel – Manage "Paths," "Welcome Screen," "Photos," and "Settings" with ease.Automatic Photo Matching – Systems matches photos to treatments via filename (supports inflectional variants) or manual override.Media Library Auto-Import – Automatically imports images from the /treatments/ directory into the WP Media Library upon activation.Customizable Color Palette – Set primary colors, accents, gradients, and form button styles to match your brand.CF7 / Elementor Integration – Seamlessly embed a consultation form at the end of the quiz.Splide Slider – A sleek, responsive carousel for treatment recommendations on the results screen.Accessibility (a11y) – Built-in ARIA labels and roles for a screen-reader friendly experience.Security First – CSRF nonces on all admin forms and full data sanitization/escaping.RequirementsWordPress: 5.8+PHP: 7.4+(Optional): Contact Form 7 or any shortcode-based form builder.InstallationDownload the repository or the ZIP package.Upload the virtual-beautician folder to /wp-content/plugins/.Activate the plugin in WordPress Dashboard → Plugins.Images from the /treatments/ folder will be automatically imported into your Media Library.Navigate to the Virtual Beautician menu to configure your paths, colors, and form settings.Place the shortcode on any page:[virtual_beautician_quiz]Configuration1. Paths (Tab: "Paths")Define your logic tree: Body Part → Concerns → Treatments.Each concern can be linked to multiple treatments using a Select2 multi-select field.2. Photos (Tab: "Photos")Automatic: The system matches images to treatments based on the filename (handles Polish/English linguistic variants like cialo / ciala).Manual: Directly assign a specific library image to a treatment.3. Settings (Tab: "Settings")OptionDescriptionTreatment SourcePost type to pull from (default: page)Parent CategoryFilter sub-pages by a specific parent pageForm ShortcodePaste your CF7, Elementor, or WPForms shortcode hereSummary FieldThe ID/Name of the form field where quiz results should be injectedColorsPrimary, gradient, accent, and button stylingMethods (Step 4)Configurable list – one line per option (e.g., Manual, Laser, Needle)File StructurePlaintextvirtual-beautician/
...
└── treatments/                   # Treatment images (imported on activation)
    ...
    ├── cryolipolysis.webp
    └── ...
Dependencies (CDN)Splide (v4.1.4): Recommendation slider.Select2 (v4.1.0-rc.0): Multi-select functionality in the admin panel.Note: Scripts are enqueued conditionally only where needed.SecurityEvery admin form is protected by nonces (wp_nonce_field / check_admin_referer).Input data is sanitized via sanitize_text_field, wp_kses_post, intval, and sanitize_hex_color.AJAX handlers are protected by check_ajax_referer and current_user_can('manage_options') capability checks.AuthorPatryk ŁyjakLicenseCopyright © 2024 Patryk Łyjak. All rights reserved.The source code is visible for inspection but is not open-source software. Viewing the code does not grant any rights to use, copy, modify, or distribute it without the express written consent of the author.Would you like me to generate a README.txt file for this plugin according to WordPress.org repository standards?
